...
def local_reading(path, format="csv", sep=",", lib="pandas"):
    ...
    if lib == "polars":
        if os.path.isfile(path):
            file = pl.read_csv(path, separator=sep, infer_schema_length=0, encoding='utf-8-sig')
            return file
        ...
        if os.path.isfile(path):
            file = pd.read_csv(path, sep=sep, on_bad_lines="skip", engine="python")
            return file
...
def Network_ditribution(df, path_output=""):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        df["domain_group"].value_counts(ascending=False).to_csv(
            os.path.join(path_output, "network_distribution_pandas.csv"), index=True
        )
    elif isinstance(df, pl.DataFrame):
...
def Repeated_posts(df, path_output=""):
    ...
    if isinstance(df, pd.DataFrame):
        df["content"].value_counts(ascending=False).to_csv(
            os.path.join(path_output, "Repeated_posts_pandas.csv"), index=True
        )
    elif isinstance(df, pl.DataFrame):
...
def hash_table(df, path_output):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        hash_cal(hashs).to_csv(
            os.path.join(path_output, "Hash_global_pandas.csv"), index=True
        )
        ...
        hash_cal(hashs).to_csv(
            os.path.join(path_output, "Hash_global_polars.csv"), index=True
        )
    else:
        print("Is not pandas or polars library")
...
def sentences_to_words(sentences):
    ...
    sentences = emoji.demojize(sentences, language="es")
...
def compute_coherence_values(
    ...
    lda_model.save(os.path.join(path_output, f"model_LDA{str(k)}_redes.model"))
...
def Lda_range(
    ...
    con_codo = 0
    with open(os.path.join(path_output, "coherence_results.pkl"), "wb") as f:
        pickle.dump(persistence, f)
    pd.DataFrame(model_results).to_csv(
        os.path.join(path_output, "coherence_results.csv"), index=True
    )
...
def lda_process(
    ...
    if k:
        ...
        df_m.to_csv(os.path.join(path_output, "Coherence_values.csv"), index=True)
        x = df_m.index.tolist()
        y = df_m["Coherence"].tolist()
        con_codo = 0
...
def write_df(df, format="csv", path_output=""):
    ...
    if format == "csv":
        if isinstance(df, pd.DataFrame):
            df.to_csv(os.path.join(path_output, "df_processed.csv"), index=False)
        elif isinstance(df, pl.DataFrame):
            df.write_csv(os.path.join(path_output, "df_processed.csv"), index=False)
...
def write_metadata(
    ...
    json_write(
        os.path.join(path_output, "metadata.json"), metadata
    )  # output of the data
...
def load_gensim_lda_model(path_models: str, NUM_TOPICS: int):
    ...
    return models.ldamodel.LdaModel.load(
        os.path.join(path_models, f"model_LDA{str(NUM_TOPICS)}_redes.model")
    )
